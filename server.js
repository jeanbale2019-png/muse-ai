...
app.post('/api/ai/analyze-image', async (req, res) => {
  ...
  try {
    ...
    // Fallback model name handling if passed incorrectly
...
app.post('/api/ai/chat', async (req, res) => {
  ...
  try {
    ...
    // Conversion de l'historique simple vers le format SDK si nécessaire
    // (Le SDK attend { role: 'user'|'model', parts: [{ text: '...' }] })
    // Mais le helper `chats.create` gère souvent le format simplifié ou on doit mapper.
    // Pour simplifier ici avec la nouvelle lib @google/genai:
    ...
    const result = await chat.sendMessage({ parts: [{ text: message }] });
