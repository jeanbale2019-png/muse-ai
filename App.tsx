...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (checkLock()) return;
    const interval = setInterval(() => checkLock() && clearInterval(interval), 1000);
  ...
  if (!isI18nReady) {
    return (
      <div className="min-h-screen bg-black flex flex-col items-center justify-center space-y-4">
        ...
        <span className="text-[10px] font-black uppercase tracking-[0.3em] text-indigo-400">Loading Dictionary...</span>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#09090b] text-[#f8fafc] flex flex-col md:flex-row overflow-x-hidden">
      ...
      <main className={`flex-1 transition-all duration-500 pb-24 md:pb-0 landscape:pb-0 ${activeTab !== 'live' && sidebarExpanded ? 'md:ml-64' : activeTab !== 'live' ? 'md:ml-20' : 'ml-0'}`}>
        ...
        {activeTab !== 'live' && (
          <header className="sticky top-0 z-40 bg-[#09090b]/80 backdrop-blur-xl border-b border-white/5 h-16 md:h-20 flex items-center justify-between px-6">
            ...
            <div className="flex items-center space-x-4">
              ...
              >
                <img src={user?.avatar || `https://api.dicebear.com/7.x/avataaars/svg?seed=${user?.id || 'default'}`} className="w-full h-full object-cover" alt="me" />
              </button>
            </div>
          </header>
        )}
        ...
        <div className={`w-full h-full ${activeTab === 'live' ? '' : 'max-w-7xl mx-auto p-4 md:p-8'}`}>
          ...
          {activeTab === 'live' && (
            user?.tier === 'premium' || user?.tier === 'business' ? (
              ...
              <div className="flex flex-col items-center justify-center py-20 text-center space-y-6">
                 ...
                 <button onClick={() => setIsAccessLocked(true)} className="px-8 py-4 bg-indigo-600 rounded-2xl font-black uppercase text-[10px] tracking-widest">Voir les offres</button>
              </div>
            )
          )}
          ...
          {activeTab === 'social' && (
            <SocialFeed 
              language={language} 
              onProfile={(uid) => {setTargetProfileId(uid); setActiveTab('profile');}} 
              user={user} 
      ...
      <div className="portrait:block landscape:hidden md:hidden">
        <MobileNavBar activeTab={activeTab} onTabChange={handleTabChange} onOpenChat={() => setIsChatOpen(!isChatOpen)} />
      </div>
